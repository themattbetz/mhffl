...
```{r}
gt_tble = bind_cols((teams %>% filter(Division == 'Nick Division') %>% arrange(Owner)),
          (teams %>% filter(Division == 'Non-Nick Division') %>% arrange(Owner))) %>% 
...
selectInput(inputId = 'name',label = 'Team',
  choices = as.vector((teams %>% select(Owner) %>% arrange(Owner))[,1]))
...
data = game_data %>% 
  ...
  summarise(Team = round(mean(Points,na.rm=TRUE),2),
            League = round(mean(League_Average_Points),2)) %>% 
  melt() %>% 
...
game_data %>% 
  ...
  summarise(Total_Points = sum(Points),
            Predicted_Points = sum(Predicted)) %>% 
  melt(id = 'Week') %>% 
  rename(Type = variable,
         Points = value) %>% 
